...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    thread::spawn(move || {
        ...
        let baud_rate = 38400; // Updated to match the 16MHz integer math fix
            ...
            Ok(port) => {
                ...
                loop {
                    ...
                    if reader.read_line(&mut line).is_ok() {
                        // Strip \r\n and any trailing whitespaces injected by dtostrf
                        let data = line.trim().trim_matches(|c: char| c == '\r' || c == '\n');
                        if data.is_empty() {
                            continue;
                        }
                        ...
                        if parts.len() == 2 {
                            if let (Ok(scan_angle), Ok(scan_dist)) = (
                                ...
                                if let Ok(mut map) = grid_for_serial.lock() {
                                    // Base is permanently locked to 0.0, 0.0, heading 0.0 for the stationary sweep
