...
pub struct OccupancyGrid {
    ...
    pub safe_spawns: Vec<(i32, i32)>, // Pre-calculated safe zones
}
...
impl OccupancyGrid {
            ...
            for dx in -INFLATION_RADIUS..=INFLATION_RADIUS {
                for dy in -INFLATION_RADIUS..=INFLATION_RADIUS {
                    if dx * dx + dy * dy <= INFLATION_RADIUS * INFLATION_RADIUS {
                        self.inflated_cells
                            .insert((grid_idx.0 + dx, grid_idx.1 + dy));
                    }
                }
            }
        }
    ...
    pub fn find_path(
        ...
        while let Some(current) = open_set.pop() {
            ...
            if !closed_set.insert(current.position) {
                continue;
            }
            ...
            if current.position == goal {
                ...
                while curr != start {
                    path.push(curr);
                    if let Some(&next) = came_from.get(&curr) {
                        curr = next;
                    } else {
                        break;
                    }
            ...
            for &(dx, dy, move_cost) in &neighbors {
                ...
                if inflated_cells.contains(&neighbor) {
                    continue;
                }
                if neighbor != start
                    && neighbor != goal
                    && *cells.get(&neighbor).unwrap_or(&0.0) >= 0.0
                {
                    continue;
                }
